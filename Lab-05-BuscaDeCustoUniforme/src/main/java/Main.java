...
//        }
        ...
        List<No> result = UCS.execute(arad, bucharest);
        assert result != null;
        for (No n : result)
            System.out.println(n);
        System.out.println();

        medirTempo(() -> UCS.execute(arad, bucharest));
        medirTempo(() -> UCS.execute(sibiu, fagaras));
        medirTempo(() -> UCS.execute(oradea, craiova));
        medirTempo(() -> UCS.execute(timisoara, bucharest));
        medirTempo(() -> UCS.execute(zerind, pitesti));
        medirTempo(() -> UCS.execute(lugoj, urziceni));
        medirTempo(() -> UCS.execute(arad, neamt));
        medirTempo(() -> UCS.execute(craiova, iasi));
        medirTempo(() -> UCS.execute(mehadia, vaslui));
        medirTempo(() -> UCS.execute(giurgiu, eforie));
